...
void Trace::SetDefaultConfig() {
	...
	glbConfig.LineCutOffDistance = 10.0f;
	glbConfig.LineCutOffAngle = 0.95;
	glbConfig.LongLineDistance = 5.0f;
...
//
void Trace::ProcessImage(std::vector<Strip *> &optStrips, unsigned char *data, int width, int height) {
	...
	//LineSegmentsToStrips(optStrips, lineSegments);

	OptimizeLineSegments(optSegments, lineSegments);
	LineSegmentsToStrips(optStrips, optSegments);
}

void Trace::ProcessImageDebug(unsigned char *data, int width, int height) {
	// I believe this is bogus
	glbConfig.Width = width;
	glbConfig.Height = height;
