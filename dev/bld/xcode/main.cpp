...
{
public:	
  virtual IPluginObject *CreateObject(yapt::ISystem *pSys, const char *guid_identifier);
};
class MyBaseEffect :
...
IPluginObject *MyEffectFactory::CreateObject(yapt::ISystem *pSys, const char *guid_identifier)
{
...
void MyResource::Render(double t, IPluginObjectInstance *pInstance)
{
	...
	// --->> TODO: problem
	IResourceContainer *pResources = pSys->GetActiveDocument()->GetResources();
...
//	pLogger->Debug("key1=%s",system->GetConfigValue("key1"));
	...
	TestExpat(system, "file://timeline_test.xml");
